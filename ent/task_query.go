...
func (tq *TaskQuery) QueryList() *ListQuery {
...
func (tq *TaskQuery) WithList(opts ...func(*ListQuery)) *TaskQuery {
	query := &ListQuery{config: tq.config}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withList; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "list_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.List = n
			}
		}
	}
